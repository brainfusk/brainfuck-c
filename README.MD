...
编写所有需要用到的nasm汇编指令的组合版，仅用作反汇编获取对应指令的二进制形式，避免查汇编指令手册。

使用nasm生成64位目标文件(目标文件会被部分优化，寄存器可能使用低位，如rax->eax)。

使用objdump -d 反汇编出相应的16进制机器码，参照反汇编代码生成jit代码存入数组即可。

## JIT运行原理

申请一段按PAGE_SIZE对齐的内存,写入jit二进制代码，调用系统调用使这段内存变为可读可执行，然后把这段内存当做函数执行即可。

- mmap + mprotect 或 memalign + mprotect (推荐)
...
> **注意**：jz/jnz指令在汇编中为标签，在机器码中为偏移地址，注意跳转指令的选择（0x0f84 0x0f85与0x74 0x75的区别），详见[跳转指令的区别](#跳转指令的区别)。

## 查看生成的机器码和反汇编代码

### 方法一：gdb查看指定范围内存数据

找到机器码存储位置，对于666.bf程序，二级制机器码存储在char* code[],length=171中。
...
以汇编格式编码指定范围内存并显示，可同时显示二进制和对应汇编代码。
...
使用-g 参数开启调试信息并运行，异常后获取生成的coredump文件。
...
# memcheck*.xml为valgrind内存泄漏检测结果文件 (make memcheck)
# cachegrind.* 为valgrind CPU缓存检测和分支预测输出(make cachegrind)
...
0x74 0x75 与 0x0f84 0x0f85 的区别在于操作数大小范围不同：
